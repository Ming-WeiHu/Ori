...
def plot_v_contour(r: PIVResults, start: int = 75, end: int = 150, ax=None) -> None:
    """Plot a contour map of the time-averaged v velocity over a frame range."""
    import matplotlib.pyplot as plt
    if ax is None:
        _, ax = plt.subplots(figsize=(8, 6))

    # Clamp indices to valid ranges to prevent out-of-bounds errors
    start_idx = max(0, start)
    end_idx = min(r.n_pairs, end)

    if start_idx >= end_idx:
        raise ValueError(f"Invalid frame range: {start} to {end}.")

    # Calculate time-averaged v velocity (in m/s) for the specified range
    import warnings
    with warnings.catch_warnings():
        warnings.simplefilter("ignore", RuntimeWarning)
        v_slice = r.v_original[start_idx:end_idx]
        v_mean_range = np.nanmean(v_slice, axis=0) * r.calv

    # Plot filled contours
    c = ax.contourf(r.x, r.y, v_mean_range, levels=30, cmap="viridis")
    ax.set_aspect("equal")
    ax.invert_yaxis()  # Image-y points down
    
    ax.set_title(f"{r.path.name} — mean v velocity (frames {start_idx}-{end_idx})")
    ax.set_xlabel("x (px)")
    ax.set_ylabel("y (px)")
    plt.colorbar(c, ax=ax, label="v velocity (m/s)")
...
def _cli() -> None:
    ...
    p.add_argument("--v-contour", action="store_true",
                   help="plot a contour map of mean v velocity over a range of frames")
    p.add_argument("--start-frame", type=int, default=75,
                   help="start frame for v contour map (default 75)")
    p.add_argument("--end-frame", type=int, default=150,
                   help="end frame for v contour map (default 150)")
    ...
    if args.pair is not None or args.mean or args.uv_curve or args.uv_curve_raw or args.v_contour:
        import matplotlib.pyplot as plt
        ...
        if args.v_contour:
            plot_v_contour(r, start=args.start_frame, end=args.end_frame)
            output_image = output_dir / f"flow_field_v_contour_{args.start_frame}_{args.end_frame}.png"
            plt.gcf().savefig(output_image, dpi=300, bbox_inches='tight')
            plt.close()
            print(f"\n✅ V contour plot saved successfully to: {output_image}")
